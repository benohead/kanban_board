...
<body>
	...
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."projects SET active=0 WHERE id=?");

	$stmt->bind_param("i", $projectid);
	$stmt->execute();
	$stmt->close();

	$successes[] = translate('Successfully deactivated the project');
}
else {
...
?>
			<div id='regbox'>
				<form name='newProject' action='<?php echo $_SERVER['PHP_SELF']; ?>' method='post'>
					...
					<p>
						<?php echo translate('Press "Deactivate" if you really want to deactivate the project'); ?>
					</p>
					<br> <input type='hidden' name='id' value='".$projectid."' />
					<p>
